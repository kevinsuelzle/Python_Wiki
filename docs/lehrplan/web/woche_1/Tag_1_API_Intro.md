...
### Tagesprojekt
Ziel des heutigen Tages ist es, eine Wetterabfrage mit Python in einer CLI Anwendung zu integrieren und informationen zum Wetter einen Stadt zu bekommen.
...
try:
    ...
    # Überprüfen, ob der Statuscode auf Erfolg hindeutet
    if response.status_code == 200:
        print("Erfolgreiche Anfrage!")
        data = response.json()
        # Datenverarbeitung ab hier
    else:
        print(f"Fehler aufgetreten: Statuscode {response.status_code}")

except requests.exceptions.RequestException as e:
    # Fängt alle Requests-bezogenen Exceptions ab
